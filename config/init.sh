...
if [ $FIRST_RUN ]; then
    ...
    cd /app/web
    drush site-install $DRUPAL_INSTALL_PROFILE -y --root=/app/web --uri=http://drupal8phpunit.lndo.site --account-name=admin --account-pass=admin --site-name=lando-drupal8-test-debugging
    cd /app/
fi
